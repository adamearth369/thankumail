...
/* -------------------- VERSION -------------------- */
const VERSION = "routes_v2026-01-31_004";
const COMMIT = process.env.RENDER_GIT_COMMIT || process.env.GIT_COMMIT || "";

/* -------------------- ROUTES MARKER -------------------- */
const ROUTES_MARKER = "sms_requires_sender_email_v1";
...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/admin/gifts/seed", async (req, res) => {
    ...
    const senderEmail = safeStr(parsed.data.senderEmail).trim() || "";
    ...
    // ✅ If SMS is used, senderEmail must be present and valid
    if (recipientPhone && !isEmail(senderEmail)) {
      return res.status(400).json({
        ok: false,
        error: "Sender email is required when sending by SMS",
        field: "senderEmail",
        version: VERSION,
        commit: COMMIT,
      });
    ...
    try {
      await db.insert(gifts).values({
        publicId,
        senderEmail: senderEmail ? senderEmail : null,
  ...
  app.post("/api/gifts", createGiftLimiter, async (req, res) => {
    ...
    const senderEmailRaw = safeStr(parsed.data.senderEmail).trim();
    const senderEmail = senderEmailRaw || null;
    ...
    // ✅ If SMS is used, senderEmail must be present and valid (return-to-sender path)
    if (recipientPhone && !isEmail(senderEmailRaw)) {
      return res.status(400).json({
        error: "Sender email is required when sending by SMS",
        field: "senderEmail",
        code: "SENDER_EMAIL_REQUIRED_FOR_SMS",
        version: VERSION,
        commit: COMMIT,
      });
    }
