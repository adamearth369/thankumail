...
/**
 * NOTE:
 * - Daily limits are enforced separately (dailyIpLimit / dailyRecipientLimit).
 * - This rate limiter is purely to stop burst/spam in a short window.
 * - We set it high enough so you can run automated tests without instantly hitting 429.
 */
const createGiftLimiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 120, // was 12 — raised so you can test daily limits without tripping burst limiter
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get(["/health", "/__health", "/api/health"], (_req, res) => {
    res.json({
      ok: true,
      routesMarker: "ROUTES_MARKER_v14_raise_create_rate_limit_for_daily_tests",
  ...
  app.get("/api/admin/status", (_req, res) => {
    res.json({
      ...
      email: {
        ...
        blockedDomainsCount: 10,
      },
      limits: {
        dailyIpLimit: 40,
        dailyRecipientLimit: 8,
